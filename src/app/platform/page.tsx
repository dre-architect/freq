...
export default function PlatformPage() {
  return (
    <div className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
      ...
      <p className="mt-6 max-w-4xl text-lg text-[#9CA3AF]">
        FREQ AI&apos;s platform sits between your existing hardware and your operational decisions — transforming
        raw sensor data into actionable cargo intelligence.
      </p>

      <section className="py-20">
        <p className="max-w-5xl text-[#9CA3AF]">
          Our platform is hardware-agnostic by design. Whether your operation uses radar, LiDAR, pressure sensors,
          or visual monitoring, FREQ AI&apos;s intelligence layer normalizes data from any source into a unified
          operational picture. The result: autonomous drafting, real-time monitoring, and verified compliance
          reporting — regardless of what hardware generates the input.
        </p>
      </section>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {[
            ...
            <div key={title} className={card}>
              ...
              <p className="mt-3 text-sm text-[#9CA3AF]">{description}</p>
              {link && (
                <Link href={link} className="mt-4 inline-block text-sm text-[#7C3AED] hover:opacity-80">
                  Learn More →
                </Link>
              )}
            </div>
      ...
      <section className="pb-20">
        <h2 className="text-3xl font-bold text-white">Simulation Control Center</h2>
        <p className="mt-4 max-w-4xl text-[#9CA3AF]">
          Built for presentation and operational review workflows: manual simulation control, synchronized digital
          twin views, and measurable performance indicators. Operators can start, pause, finish, and restart each
          sequence with no automatic playback.
        </p>
        <div className="mt-8">
          <SimulationConsole />
        </div>
      ...
      <section className="pb-20">
        <div className="rounded-lg border border-[#1E293B] bg-[#111827] p-12 text-center">
          <h2 className="text-3xl font-bold text-white">See the Platform in Action</h2>
          <p className="mx-auto mt-4 max-w-2xl text-[#9CA3AF]">
            Demonstrate real-time drafting intelligence, synchronized data rendering, and operational optimization in
            a seamless multi-view experience designed for executives, operators, and technical teams.
          </p>
          ...
          <p className="mt-6 text-sm text-[#9CA3AF]">Simulation demo coming soon.</p>
        </div>
      </section>

      <section>
        <div className="rounded-lg border border-[#1E293B] bg-[#111827] p-8 text-center">
          <p className="text-xl text-white">Ready to see what FREQ AI can do for your operation?</p>
          <Link
            href="/contact"
            className="mt-5 inline-block rounded-lg bg-[#7C3AED] px-6 py-3 font-medium text-white hover:bg-[#6D28D9]"
          >
            Contact Us
          </Link>
        </div>
